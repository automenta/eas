#!/usr/bin/env python3
"""
phi2_learned_vector.py - "Activation Fine-Tuning"
...
def main():
    ...
    for i, ex in enumerate(dataset):
        ...
        # For training, we need the full text to compute loss?
        # Actually, standard causal LM training minimizes loss on the NEXT token.
        # So we want to minimize loss on the token corresponding to the correct label (A, B, C, or D).
    ...
    for epoch in range(epochs):
        ...
        for i, s in enumerate(train_set):
            # Prepare input
            # We want to predict the ANSWER token given the PROMPT
            inputs = tokenizer(s["prompt"], return_tensors="pt", truncation=True, max_length=512).to(device)
